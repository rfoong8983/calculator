...
describe('when building a number', () => {
    ...
    describe('it handles decimals properly', () => {
        ...
        test('it correctly handles trailing zeros', () => {
            const calc = new CalculatorModel();
...
describe('adds values to a stack', () => {
    ...
    describe('when the value is a number', () => {
        ...
        test('it does only shows strings of length: 18 (without decimals)', () => {
            const calc = new CalculatorModel();
            for (let i = 0; i < 20; i++) {
                calc.appendToBuilder('1');
            }
            expect(calc.numberBuilder).toHaveLength(18);
            expect(calc.numberBuilder).toEqual('111111111111111111');
        });

        test('it does only shows strings of length: 18 (including decimals)', () => {
            const calc = new CalculatorModel();
            for (let i = 0; i < 20; i++) {
                if (i !== 5) {
                    calc.appendToBuilder('1');
            ...
            expect(calc.numberBuilder).toHaveLength(18);
            expect(calc.numberBuilder).toEqual('11111.111111111111');
        });
    });
...
describe('if the button value is equals', () => {
    ...
    describe('if the button pressed after equals is an operation', () => {
        test('it should continue calculations (add)', () => {
            const calc = new CalculatorModel();
            calc.performOp('+');
            calc.appendToBuilder('2');
            calc.performOp('*');
